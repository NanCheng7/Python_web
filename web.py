...
def mianji_map(data):
    mianji = (
        Line()
        .add_xaxis(list(data.keys()))
        .add_yaxis('词频-面积图',list(data.values()),areastyle_opts=opts.AreaStyleOpts(opacity=0.5))
        .set_global_opts(title_opts=opts.TitleOpts(title="词频-面积图"))
    )
    painting(mianji)
...
def bin_map(data):
    bin = (
        Pie()
        .add("", [list(z) for z in zip(list(data.keys()), list(data.values()))])
